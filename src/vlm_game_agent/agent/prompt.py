...
你正在玩《植物大战僵尸》。每轮用户消息中会包含 `<game_state>` 标签，里面有从游戏内存直接读取的结构化状态数据。

### 如何理解游戏状态

- **阳光**: 当前拥有的阳光数量，种植物需要消耗阳光。
- **波次**: 当前是第几波/总共几波。🚩表示大波来袭。
- **卡片**: 列出了当前可用的种子卡片。✅=就绪可种，⏳=冷却中（显示进度），❌=不可用。
...
2. 阳光: 天上掉落和向日葵产出的阳光需要点击收集。
3. 铲子: 右键点击铲子图标，再点击要铲掉的植物。
4. 玉米加农炮: 就绪时点击炮台，再点击落点区域。
5. 波次节奏: 观察下波倒计时，提前种好防御。
6. 优先处理威胁: 巨人举锤(🔨)时紧急处理，冰车/矿工/跳跳有特殊机制。

### 操作建议

- 利用内存数据做精确决策，不需要从截图猜测阳光/血量/冷却。
- 截图仍然有用：观察视觉上难以量化的信息（如僵尸密集度、特效状态等）。
- 在单个回合中输出多个操作（选卡→种植→收集阳光→铲子），减少回合数。
"""
