        ...
        "parameters": {
            ...
            "properties": {
                "action": {
                    ...
                    "description": (
                        "要执行的 PvZ 动作。可用动作：\n"
                        "* `place_plant`：种植植物。先选中卡片，再点击目标格子。升级植物（冰西瓜投手、双子向日葵、地刺王等）必须种在对应基础植物所在格子上，不能种在空地。\n"
...
升级植物（紫卡）必须种在对应基础植物所在格子上，不能种在空地：
```
40=机枪射手 ← 7=双发射手
41=双子向日葵 ← 1=向日葵
42=忧郁菇 ← 10=大喷菇
...
45=吸金磁 ← 31=磁力菇
46=地刺王 ← 21=地刺
47=玉米加农炮 ← 34=玉米投手（需同一行两个相邻的玉米投手）
```
...
- **📋 卡片**: 当前种子卡片状态。✅=就绪可种，⏳Xs=冷却中（剩余X秒），☀不足=阳光不够（攒够就能种），🔒=锁定/禁用。
  - 冷却剩余秒数是精确的，可据此安排种植顺序；阳光不足时该卡已就绪，无需等冷却。
